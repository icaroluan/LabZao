...
A solução deve ser enviada para o e-mail <rodrigo@dcc.ufba.br> até as 18:00:00
da segunda-feira, dia 30 de novembro de 2015 (horário local de Salvador). O e-mail
...
int menor(Heap *heap, int pos1, int pos2) {
  // TODO
  int x = heap->itens[pos1].tempo;
  int y = heap->itens[pos2].tempo;
...
int ehHeapMin(Heap *heap) {
  ...
  int x = 1, minimo = 0;
    do{
        if(heap->itens[minimo].tempo <= heap->itens[x].tempo && heap->itens[minimo].id <= heap->itens[x].id){
            minimo = x;
            x++;
        }
      else
...
void afunda(Heap *heap, int pos) {
  // TODO
  int aux = pos;  
    if(indiceFilhoEsq(pos) < heap->tamanho){
        if(heap->itens[indiceFilhoEsq(pos)].tempo < heap->itens[aux].tempo)
            aux = indiceFilhoEsq(pos);
         
        if(heap->itens[indiceFilhoEsq(pos)].tempo == heap->itens[aux].tempo)
            if(menor(heap, indiceFilhoEsq(pos), aux))
                aux = indiceFilhoEsq(pos);
     
    }
    if(indiceFilhoDir(pos) < heap->tamanho){
        if(heap->itens[indiceFilhoDir(pos)].tempo < heap->itens[aux].tempo)
            aux = indiceFilhoDir(pos);
 
        if(heap->itens[indiceFilhoDir(pos)].tempo == heap->itens[aux].tempo)
           if(menor(heap, indiceFilhoDirq(pos), aux))
                aux = indiceFilhoDir(pos);
    }  
 
    if(pos != aux){
        troca(&heap->itens[pos], &heap->itens[aux]);
        afunda(heap, aux);
    }
}
...
int main(int argc, char *argv[]) {
  ...
  afunda(heap, 0);
  corrige(3, comparaHeap(heap,6, 1,1, 3,3, 2,2, 6,6, 4,4, 5,5));

  preencheHeap(heap,6, 6,6, 1,1, 2,2, 4,4, 3,3, 5,5);
  afunda(heap, 0);
  corrige(3, comparaHeap(heap,6, 1,1, 3,3, 2,2, 4,4, 6,6, 5,5));
