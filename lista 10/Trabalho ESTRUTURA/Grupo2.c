...
int ehHeapMin(Heap *heap) {
  // TODO
  // fesq = left, fdir = right, i = x
  int x, minimo = 0, left = indiceFilhoEsq(x), right = indiceFilhoDir(x);
  do{
...
void afunda(Heap *heap, int pos) {
  // TODO
  int left = indiceFilhoEsq(pos), right = indiceFilhoDir(pos), afunda_pos = pos;
  if(left < heap->tamanho){
    if(heap->itens[left].tempo < heap->itens[afunda_pos].tempo){
      afunda_pos = left;
    }
  }
/////////////////////////////////////////////////////////////////////////////  
  if(left < heap->tamanho){
    if(heap->itens[left].tempo == heap->itens[afunda_pos].tempo){
      if(heap->itens[left].id < heap->itens[afunda_pos].id){
        afunda_pos = left;
      }
    }
  } 
/////////////////////////////////////////////////////////////////////////////  
  if(right < heap->tamanho){
    if(heap->itens[right].tempo < heap->itens[afunda_pos].tempo){
      afunda_pos = right;
    }
  }
/////////////////////////////////////////////////////////////////////////////  
  if(right < heap->tamanho){
    if(heap->itens[right].tempo == heap->itens[afunda_pos].tempo){
      if(heap->itens[right].id < heap->itens[afunda_pos].id){
        afunda_pos = right;
      }
    }
  }
/////////////////////////////////////////////////////////////////////////////  
  if(pos != afunda_pos){
    troca(&heap->itens[pos], &heap->itens[afunda_pos]);
    afunda(heap, afunda_pos);
  }
}
